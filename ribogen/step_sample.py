...
class FixSampler(ScheduleSampler):
    def __init__(self, diffusion):
        ...
        self._weights = np.concatenate(
            [np.ones([diffusion.num_steps//2]), np.zeros([diffusion.num_steps//2]) + 0.5])
...
class LossSecondMomentResampler(LossAwareSampler):
    def __init__(self, diffusion, history_per_term=10, uniform_prob=0.001):
        ...
        self._loss_history = np.zeros(
            [diffusion.num_steps, history_per_term], dtype=np.float32
        )
        self._loss_counts = np.zeros([diffusion.num_steps], dtype=np.int32)

    def weights(self):
        if not self._warmed_up():
            return np.ones([self.diffusion.num_steps], dtype=np.float32)
